const PLAN = {
  ...
  Friday:{title:"Lower B + Core",type:"Strength",exercises:[
    ["Bulgarian split squat","3 sets × 8–15 / leg","Quads • glutes","Add reps before adding backpack weight."],
...
function renderToday(){
  ...
  todayTitle.textContent=p.title; todaySubtitle.textContent=p.type==="Rest"?"Recovery day — no workout required.":p.type==="Recovery"?"Keep it easy today.":`${p.exercises.length} exercises • 45–60 min`;
  ...
  p.exercises.forEach((ex,i)=>{
    ...
    for(let s=0;s<n;s++){
      const val=l.sets[s]??"";
      const box=document.createElement("div"); box.className="set"+(l.sets[s]!=null&&l.sets[s]!==""?" done":"");
      box.innerHTML=`<label>SET ${s+1}</label><input type="number" min="0" inputmode="numeric" value="${escapeHtml(val)}" placeholder="reps"><button>${l.sets[s]!=null&&l.sets[s]!==""?"✓ Logged":"Log set"}</button>`;
      const input=box.querySelector("input"), btn=box.querySelector("button");
      btn.onclick=()=>{const x=getLog(date,ex[0]);x.sets[s]=input.value;data.logs[logKey(date,ex[0])]=x;save()};
      sets.appendChild(box);
    }
    const article=t.querySelector(".exercise");
    const doneBtn=document.createElement("button"); doneBtn.className="secondary"; doneBtn.textContent=l.completed?"✓ Exercise complete":"Mark exercise complete";
    doneBtn.style.marginTop="10px"; doneBtn.onclick=()=>{const x=getLog(date,ex[0]);x.completed=!x.completed;data.logs[logKey(date,ex[0])]=x;save()};
    article.appendChild(doneBtn);
    todayContent.appendChild(t);
  });
  const complete=document.createElement("button"); complete.className="primary"; complete.textContent="✓ Complete today's workout";
  complete.onclick=()=>{data.logs[`DAY__${date}`]={completed:true};save();alert("Workout marked complete. Nice.");};
...
function renderPlan(){
  ...
  DAYS.slice(1).concat(["Sunday"]).forEach(day=>{
    const p=PLAN[day], div=document.createElement("div"); div.className="dayCard";
    ...
    p.exercises.forEach(e=>div.innerHTML+=`<div class="exerciseLine"><span><b>${escapeHtml(e[0])}</b><br><span class="muted">${escapeHtml(e[2])}</span></span><span class="muted">${escapeHtml(e[1])}</span></div>`);
    planContent.appendChild(div);
  });
}

function renderStats(){
  const days=new Set(), workouts=new Set(), setCount=Object.values(data.logs).reduce((a,l)=>{
    if(l.sets) a+=l.sets.filter(x=>x!==""&&x!=null).length; return a;
  },0);
  Object.keys(data.logs).forEach(k=>{
    if(k.startsWith("DAY__")) {const d=k.slice(5); if(data.logs[k].completed){days.add(d);workouts.add(d)}}
  });
  workoutsDone.textContent=workouts.size; daysDone.textContent=days.size; totalSets.textContent=setCount;
  let streak=0, d=new Date();
  while(true){const k=`DAY__${iso(d)}`;if(data.logs[k]?.completed){streak++;d.setDate(d.getDate()-1)}else break}
  currentStreak.textContent=streak;
}

function renderHistory(){
  history.innerHTML=data.checkins.slice().reverse().map(c=>`<div class="checkin"><b>${c.date}</b><div class="muted">Weight ${c.weight||"—"} kg • Waist ${c.waist||"—"} cm • Chest ${c.chest||"—"} cm • Arm ${c.arm||"—"} cm • Push-ups ${c.pushups||"—"} • Plank ${c.plank||"—"} sec</div></div>`).join("")||`<div class="card"><p>No check-ins yet. Do your baseline before starting.</p></div>`;
}
function renderPhotos(){
  photoList.innerHTML=data.photos.slice().reverse().map((p,i)=>`<div class="photo"><span><b>${escapeHtml(p.date)}</b><br><span class="muted">${escapeHtml(p.filename)}</span></span><button class="secondary" onclick="deletePhoto(${data.photos.length-1-i})">Delete</button></div>`).join("")||`<div class="card"><p>No photo records yet.</p></div>`;
}
...
saveBaseline.onclick=()=>{
  data.checkins.push({date:iso(),weight:weightInput.value,waist:waistInput.value,chest:chestInput.value,arm:armInput.value,pushups:pushupInput.value,plank:plankInput.value});
  ["weightInput","waistInput","chestInput","armInput","pushupInput","plankInput"].forEach(id=>document.getElementById(id).value="");
  save();
};
savePhoto.onclick=()=>{
  ...
  photoFilename.value="";save();
};
function deletePhoto(i){data.photos.splice(i,1);save()}
resetBtn.onclick=()=>{
  if(confirm("Delete ALL tracker data from this browser?")){localStorage.removeItem(KEY);data={logs:{},checkins:[],photos:[]};renderAll()}
};
renderAll();
